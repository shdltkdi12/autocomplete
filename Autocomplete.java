...
public class Autocomplete {
   ...
   /**
     ...
     * 
     * @author Austin Lu, Jeff Forbes, Isaiah Roh
     *
     */
     ...
     * 
     * @author Austin Lu, Jeff Forbes, Isaiah
     *
     */
    public static class TrieAutocomplete implements Autocompletor {
        ...
        private void add(String word, double weight) {
            ...
            double max = 0.0;
            if(myRoot.mySubtreeMaxWeight == 0) {
               Node node = new Node(word.charAt(0), myRoot, weight);
               myRoot.children.put(word.charAt(0), node);
               Node current = node;
               for(int i = 1; i < word.length(); i++) {
                  Node n = new Node(word.charAt(i), current, weight);
               ...
               for(int i = 0; i < word.length(); i++) {
                  for(Map.Entry<Character, Node> entry: current.children.entrySet()) {
                     if(entry.getValue().mySubtreeMaxWeight > max) {
                        break;
                     }
                     else if(entry.getValue().mySubtreeMaxWeight <= max){
                        flag = true;
                     }
                  }
                  if(flag) {
               ...
               current.setWord(word);
               current.setWeight(weight);
               current.mySubtreeMaxWeight = current.getWeight();
               current = current.parent;
               max = weight;
               boolean flag = false;
               for(int i = 0; i < word.length(); i++) {
       ...
       public Iterable<String> topMatches(String prefix, int k) {
               ...
               while(!q.isEmpty()){
                  ...
                  for(Map.Entry<Character, Node> entry: current.children.entrySet()) {
                     ...
                     if(entry.getValue().isWord) {
                        map.put(entry.getValue().getWord(), entry.getValue().getWeight());
                     }
                  }
               }
               Double max = 0.0;
           ...
           for(Map.Entry<Character, Node> entry: current.children.entrySet()) {
            if(entry.getValue().myInfo.charAt(0) == prefix.charAt(0)) {
              q.add(entry.getValue()); 
            }
           }
           ...
           while(!q.isEmpty()) {
            ...
            for(Map.Entry<Character, Node> entry: current.children.entrySet()) {
               q.add(entry.getValue());
               if(entry.getValue().isWord && entry.getValue().getWord().length() >= length) {
                  if(entry.getValue().getWord().substring(0, length).equals(prefix)) {
                     map.put(entry.getValue().getWord(), entry.getValue().getWeight());
                  }
               }   
             }
           ...
           Iterable<String> iterable = Arrays.asList(output);           
           return iterable;
       }
        ...
        public String topMatch(String prefix) {
            ...
            if(prefix == "") {
               Node current = myRoot;
               double max = myRoot.mySubtreeMaxWeight;
               boolean flag = false;
            ...
            for (int i = 0; i < prefix.length(); i++) {
                 if (!current.children.containsKey(prefix.charAt(i))) {
                  ...
                  if(i == prefix.length()-1) { 
                     max = current.mySubtreeMaxWeight;
                  }
            }
            first = myRoot;
            first = first.children.get(prefix.charAt(0));
            if(current.isWord && first.mySubtreeMaxWeight == current.getWeight()) {
               return prefix;
            }
            boolean flag = false;
            while(!current.children.isEmpty()) {
